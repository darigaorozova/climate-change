...
def train_co2_model():
    """Обучение модели для прогнозирования CO2"""
    try:
        df = pd.read_csv(os.path.join(PROCESSED_DATA_DIR, "climate_combined.csv"))
        df = df.dropna(subset=['Year', 'CO2_Level'])
        
        X = df[['Year']].values
        y = df['CO2_Level'].values
        ...
        # Сохраняем модель
        model_path = os.path.join(BASE_DIR, 'models', 'co2_model.pkl')
        os.makedirs(os.path.dirname(model_path), exist_ok=True)
        with open(model_path, 'wb') as f:
            pickle.dump(model, f)
        
        print("✅ CO2 model trained")
        return model
    except Exception as e:
...
def predict_co2(years):
    """Прогнозирование CO2 на будущие годы"""
    try:
        model_path = os.path.join(BASE_DIR, 'models', 'co2_model.pkl')
        if not os.path.exists(model_path):
            train_co2_model()
        ...
        print(f"❌ Error predicting CO2: {e}")
        return None
...
def generate_predictions():
    ...
    try:
        ...
        co2_predictions = predict_co2(future_years)
        
        predictions_df = pd.DataFrame({
            'Year': future_years,
            'Predicted_Temp': temp_predictions,
            'Predicted_CO2': co2_predictions
        })
